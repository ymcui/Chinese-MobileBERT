...
class CSCProcessor(DataProcessor):
  """Processor for the ChnSentiCorp."""
...
class THUCNewsProcessor(DataProcessor):
  """Processor for the THUCNewsProcessor."""
  ...
  def get_labels(self):
    ...
    label_list_out = [tokenization.convert_to_unicode(x) for x in label_list]
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    if FLAGS.do_test:
      eval_file = os.path.join(FLAGS.tfrecord_dir or FLAGS.output_dir, "test.tf_record")
    else:
      eval_file = os.path.join(FLAGS.tfrecord_dir or FLAGS.output_dir, "eval.tf_record")
